...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  //   secure: req.secure || req.headers['x-forwarded-proto'] === 'https',
  // })
  // res.setHeader('Set-Cookie', `jwt=${token};HttpOnly;Secure;SameSite=None;Max-Age=${process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 1000}`)
...
exports.androidLogin = catchAsync(async (req, res, next) => {
...
exports.androidSignup = catchAsync(async (req, res, next) => {
  if (req.body.role === 'admin') return next(new AppError('You are not allowed to create an admin account', 403))
  const newUser = await User.create(req.body)
